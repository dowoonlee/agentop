...
#
#   🐳 만은 행에서 더하지 않고 컨테이너 캐시를 자리(working_dir) 단위로 다시 센다.
#   같은 자리에 세션이 둘이면 두 행 모두 그 자리의 개수를 배지로 달고 있어(그건
#   맞다 — 각 세션 옆에 자기 자리 스택이 보여야 한다), 행을 가로질러 더하는 순간
#   같은 컨테이너를 두 번 세기 때문이다.
#
...
stats() {
  ...
  awk -F'\037' -v AV="$avail" -v CACHE="$cache" \
    ...
    { ns++; ag += $15; sh += $16; mon += $17; srv += $19
      if ($21 != "") sess[$21] = 1       # 세션이 실제로 앉아 있는 자리 (배지의 귀속 기준)
      if ($12 != "") proj[$12] = 1
      if ($13 != "") wt++
      if ($18 != "") mdl[$18]++ }
    END {
      if (ns == 0) { printf ""; exit }
      # 컨테이너를 자리 단위로 가른다 — 세션이 앉아 있는 자리면 귀속(dkr), 아무도
