...
# ---------------------------------------------------------------------------
srv_ports_r() {   # <세션pid> → "19111,5173" (없으면 빈 값)
  local pid="${1:-}" rest
  _r=""
...
srv_ports() { srv_ports_r "${1:-}"; printf '%s' "$_r"; }

srv_count_r() {   # <포트목록> → 개수. 콤마 개수 + 1 (프로세스 없이 센다)
  local ports="${1:-}" rest n
  [[ -n "$ports" ]] || { _r=0; return 0; }
  n=1; rest="$ports"
  while [[ "$rest" == *,* ]]; do rest="${rest#*,}"; n=$(( n + 1 )); done
  _r="$n"
}
srv_count() { srv_count_r "${1:-}"; printf '%s' "$_r"; }

srv_badge() {
  local ports n
  srv_ports_r "${1:-}"; ports="$_r"; [[ -n "$ports" ]] || return 0
  srv_count_r "$ports"; n="$_r"
...
dkr_count() { dkr_count_r "${1:-}"; printf '%s' "$_r"; }
