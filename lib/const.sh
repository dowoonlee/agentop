...
HDLS_IND=2                      # 자식 행 한 단 들여쓰기 폭. 아이콘+state 블록이 8칸
                                #   고정이라 이 2칸은 흡수할 데가 없다 — 자식 행에서는
                                #   뒤의 act/ctx/dir 열이 통째로 그만큼 밀린다. 열을
                                #   지키는 길은 1칸(블록 안에서 흡수)이나 state 단어를
                                #   빼는 것뿐인데, 둘 다 '한눈에 자식으로 보인다' 를
                                #   잃는다. 여기서는 소속이 열 정렬보다 먼저다.
HDLS_IND_MAX=6                  # 들여쓰기 상한(3단). 자식이 또 -p 를 부르는 중첩에서
                                #   끝없이 들여쓰면 2단 화면의 dir 컬럼이 오른쪽으로
                                #   밀려 나간다 — 그 전에 멈춘다.
