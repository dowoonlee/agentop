...
ORANGE=$'\e[38;5;209m'          # Claude Code 구분색 (테라코타 계열 주황). 보드 배지의
                                #   주황(BOARDC 173)보다 밝다 — 둘이 한 행에 같이 설 수
                                #   있지만 agent 컬럼과 워크트리 뒤로 멀리 떨어져 선다.
...
VT=$'\013'
META_IND='    '                 # 2행 들여쓰기
META_BR_MAX=30                  # 2행 브랜치명 최대 길이 — 넘으면 잘라서 모드 자리 확보
MODEL_W=8                       # 2행 모델 컬럼 최소폭 ('fable5.1'·'haiku4.5' 가 8).
                                #   모델이 브랜치보다 앞이고 폭이 고정이라, 브랜치도
                                #   모든 행에서 같은 자리에 선다. 실제 폭은 그 순간
                                #   가장 긴 모델에 맞춰 늘어난다(fit_dir) — dir 컬럼과
                                #   같은 방식이고, 최소폭을 두는 이유도 같다: 흔한
                                #   이름들(opus5/sonnet5/fable5.1)에서 폭이 고정돼
                                #   세션이 뜨고 질 때마다 브랜치가 흔들리지 않는다.
...
# ---- agent 컬럼 (1행 두 번째 칸) ----
# 이 자리엔 원래 상태 단어(busy/wait/idle/stop/hdls)가 있었다. 그런데 그 값은 바로
# 앞의 아이콘(●◐○⊘↳)과 색이 이미 말하고 있어서, 5칸을 쓰면서 새로 알려 주는 게
# 하나도 없었다. 반대로 '무슨 에이전트인가' 는 아이콘 모양 하나(●/◆/◈)에만 걸려
# 있었고, busy 일 땐 셋 다 노랑이라 ◆ 와 ◈ 의 점 하나 차이만 남아 사실상 안 갈렸다.
# 그래서 자리를 맞바꿨다 — 아이콘은 상태, 이 컬럼은 에이전트.
# 상태 집계(busy N · wait N …)는 footer 의 summary 가 그대로 맡는다.
AGENT_W=6                       # 'CLAUDE'/'CURSOR' 가 6, 'CODEX' 는 5 → 6 고정.
                                #   HITL 강조 배지도 icon(1)+공백(1)+이 폭에 맞춘다.
# 이름은 각자의 구분색으로 찍는다 — 모양(●/◆/◈)만으로는 폰트에 따라 ◆ 와 ◈ 가
# 안 갈리고, 아이콘 색은 상태가 이미 쓰고 있어 정체를 실을 자리가 여기뿐이다.
# 정지·headless 행에서는 이 색 대신 그 행의 죽인 톤(STOPC/HDLSC)을 쓴다 — 이름만
# 밝게 남으면 '한 톤 죽여 눈이 먼저 가지 않게' 하려던 게 도로 풀린다.
AGENT_CLAUDE='CLAUDE'; AGENT_CLAUDE_C="$ORANGE"
AGENT_CURSOR='CURSOR'; AGENT_CURSOR_C="$CYAN"
AGENT_CODEX='CODEX';   AGENT_CODEX_C="$MAGENTA"
...
ACT_L=$'\001'                   # 여는 마커
ACT_R=$'\002'                   # 닫는 마커
MDL_L=$'\006'                   # 2행 모델 셀 여는 마커 (ACK)
MDL_R=$'\026'                   # 2행 모델 셀 닫는 마커 (SYN)
...
HDLS_IND=2                      # 자식 행 한 단 들여쓰기 폭. 들여쓰기는 행 머리가 아니라
                                #   dir 셀 '안' 에서 먹는다 — 행째 밀면 뒤의 act/ctx/dir
                                #   열이 부모 행과 어긋나 '모든 행에서 같은 열' 이 깨진다.
                                #   dir 은 고정폭이라 안에서 2칸을 내줘도 그 뒤(워크트리·
                                #   상태 사유)는 제자리에 선다. 소속과 열 정렬을 둘 다 지킨다.
HDLS_IND_MAX=6                  # 들여쓰기 상한(3단). 자식이 또 -p 를 부르는 중첩에서
                                #   끝없이 들여쓰면 dir 컬럼이 들여쓰기로만 채워진다 —
                                #   그 전에 멈춘다.
